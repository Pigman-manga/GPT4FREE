...
class AsyncImagine:
    ...
    def __init__(self, restricted: bool = True):
        ...
        self.key_header = get_header("SMD39RQ6VJ6DT96EPHO356J88EKJ8K40B6DH5OSGJEITWOZK1A")
        self.version = 1
    ...
    async def sdprem(
        ...
        try:
            response = await self._request(
                method="POST",
                url=f"https://api.vyro.ai/v1/imagine/web/generations",  # /sdprem (premium), /sd (free)
                data={
                    "model_version": self.version,
                    "prompt": prompt + (style.value[3] if style else ""),
                    "style_id": style.value[0] if style else model.value[0],
    ...
    async def controlnet(
        ...
        try:
            response = await self._request(
                method="POST",
                url=f"https://api.vyro.ai/v1/imagine/web/edits/remix",
                data={
                    "model_version": self.version,
                    "prompt": prompt + (style.value[3] if style else ""),
                    "negative_prompt": negative,
